(ns peopleserver.handler
  (:require [compojure.core :refer [defroutes routes wrap-routes]]
            [peopleserver.routes.home :refer [home-routes]]

            [peopleserver.public.websocket :as websocket]
            
            [peopleserver.middleware :as middleware]
            [peopleserver.session :as session]
...
(defn init
  ...
  (timbre/set-config!
    ...
    {:path "peopleserver.log" :max-size (* 512 1024) :backlog 10})

  (if (env :dev) (parser/cache-off!))
  (start-nrepl)
  ;;start the expired session cleanup job
  (session/start-cleanup-job!)
  (timbre/info "\n-=[ peopleserver started successfully"
...
(defn destroy
  ...
  []
  (timbre/info "peopleserver is shutting down...")
  (stop-nrepl)
